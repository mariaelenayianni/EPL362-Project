...
public class LoginDialog extends JDialog implements FreeRegistrationEventListener, RequestAccountEventListener {
	...
	public boolean isValidLogin(boolean b) throws MalformedURLException {
		if (b == true){
			parenta.dispose();
			System.out.println(wsclient.globals.role);
			if(wsclient.globals.role.equals("Legal Staff")) {
				new wsclient.legalStaffUI();
			}else if(wsclient.globals.role.equals("Legal Record Staff")) {
				new wsclient.RecordMenu();
			}else if(wsclient.globals.role.equals("Receptionist")) {
				new wsclient.ReceptionistMenu();
			}else if(wsclient.globals.role.equals("Head Office Management")) {
				new wsclient.ManagementMenu();
			}
